...
- **文件管理** — 支持进入目录、受根路径约束的返回上级、刷新、新建文件夹和确认删除
- **同名策略** — 传输前可选择覆盖、跳过或自动重命名；默认自动重命名
- **自动刷新** — 传输任务结束后自动刷新对应的本地或远程目录
- **USB 联动** — 检测到 Android USB 设备时自动连接，拔出后自动取消关联任务并断开
...
应用检测到 Android USB 设备后也会按当前选择的协议自动尝试连接；仍可使用工具栏
中的按钮手动连接、取消连接或断开。
...
工具栏中的“同名文件”菜单控制上传和下载冲突：覆盖会替换已有文件，跳过不会
创建任务，自动重命名会生成 `文件名 (1).扩展名`。为了避免误删，文件和文件夹
删除前都会显示确认提示。
...
- ADB 特殊文件名列表和 Shell 转义测试
- 删除/新建目录 UI、同名策略、完成后刷新及根目录导航的构建检查
...
- MTP 没有通用的原子替换操作；选择“覆盖”时会先删除旧文件再上传，若传输中断，
  目标文件可能暂时不存在。重要文件建议使用默认的“自动重命名”
- USB 自动检测依赖常见 Android 厂商 Vendor ID；未识别设备仍可手动连接
